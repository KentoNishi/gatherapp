...
function loadInfo(e){
	if(e==null){
		...
		write("GatherApp", [{text:"By Kento Nishi"}],
		      [{href:"window.location='https://github.com/KentoNishi/gatherapp/';",text:"View on GitHub"}]);//{href:"window.location='mailto:kento24gs@gmail.com';",text:"Send Email"},
	}
}
...
function requestEvent(id,title,loc,date,place,duration,cancel){
	...
	contents.push({html:"<input style='font-size:2.5vh;' type='datetime-local' onchange='showDate();'></input>"+
				...
				try{
					if(event.val()==null){
						...
						if(member!=null){
							...
							link.unshift({html:"<span style='font-size:3.5vh;'><a href='#' onclick="+'"'+href+';return false;"'+">Invite</a></span>"});
						}
						if(firstload){
