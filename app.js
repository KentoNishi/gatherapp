...
function loadGatherUp(id){
	...
	firebase.database().ref("gatherups/"+id+"/info").once("value",function(gather){
		firebase.database().ref("gatherups/"+id+"/members/"+uid).once("value",function(member){
			...
			if(navigator.share){
				contents.push({html:"<a href="+'"'+"navigator.share({title: '"+gather.val().title+"'+' - GatherApp', text: 'Join '+'"+gather.val().title+"'+' on GatherApp!', url: 'https://kentonishi.github.io/gatherapp#"+id+"'})"+'"'+">Share on Social Media</a>"});
			}
			contents.push({html:""});
