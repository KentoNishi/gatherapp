	...
	firebase.database().ref("gatherups/"+id+"/info").once("value",function(gather){
		firebase.database().ref("gatherups/"+id+"/members/"+uid).once("value",function(member){
			try{
				...
				var contents=[{text:date||"Unknown Date"},{text:addr!=null?addr.split(",").slice(1,addr.split(",").length-2).join(","):"Unknown Location"}];
				var check="checked";
				if(value<0){
					value=(-value);
					check="";
	...
	firebase.database().ref("users/"+uid+"/gatherups").once("value",function(gathers){
		...
		gathers.forEach(gather=>{
			firebase.database().ref("gatherups/"+gather.key+"/info").once("value",function(gatherup){
				...
				write(gatherup.val().title,[{text:(gatherup.val().date==null?"Unknown Date":date)},{text:addr!=null?addr.split(",").slice(1,addr.split(",").length-2).join(","):"Unknown Location"}],null,"loadGatherUp('"+gather.key+"');");
			});
		});
	});
}
