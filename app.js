...
window.onerror = function myErrorHandler(errorMsg, url, lineNumber) {
        if (navigator.online) {
                write("App Error", [{
                        text: "GatherApp encountered an error."
                }, {
...
function loadInfo(e) {
        if (e == null) {
                ...
                write("GatherApp", [{
                        ...
                        href: "window.location='https://github.com/KentoNishi/gatherapp/';",
                        text: "View on GitHub"
...
function menu() {
        ...
        write(name, [{
                html: "<img style='width:30vw;height:30vw;' src='" + pic + "' class='pic'></img>"
        }, {
                text: "Standard User"
        }], [{
                href: "signOut();",
        ...
        write("Search Events", [{
                html: "<input maxlength='50' class='search' placeholder='Title/Location'></input>"
        }, {
        ...
        if (e == null) {
                ...
                install.userChoice.then((choiceResult) => {
                        if (choiceResult.outcome === 'accepted') {
                                write("App Pinned!", [{
                                                text: "GatherApp was pinned to your home screen."
                                        }],
...
function cancelEvent(id, confirmed) {
        if (confirmed != null) {
                firebase.database().ref("events/" + id + "/info").update({
                        cancel: 1
                }).then(function() {
                        loadEvent(id);
                });
...
function requestEvent(id, title, loc, date, place, duration, cancel) {
        ...
        var extra = "";
        contents.push({
                html: "" + extra + "<div class='inputs'><input maxlength='50' placeholder='Title'></input>"
        });
        contents.push({
                html: "<input maxlength='50' placeholder='Address/Location' onfocus='this.setSelectionRange(0, this.value.length);" + "'></input>"
        });
        contents.push({
                html: "<input style='font-size:2.5vh;' type='datetime-local' onchange='showDate();'></input>" +
                        "<strong><span style='font-size:3.5vh;color:#2e73f7;' class='showdate'></span></strong>"
        });
        contents.push({
                html: "<input style='width:10vh;text-align:center;' type='number' min='0' " +
        ...
        contents.push({
                html: "<div class='iframe' style='display:none;'><br />" +
        ...
        contents.push({
                html: "<button onclick='" + ((id == null) ? "newEvent();" : "saveEvent(" + '"' + id + '"' + ");") + "'>" +
                        (id != null ? "Save" : "Schedule") + "</button>"
        });
        write(((id == null) ? "New" : "Edit") + " Event", contents, [{
                href: ((id == null) ? ("history.go(-1);") : ("loadEvent('" + id + "');")),
                text: "Cancel"
        }], null, "eventInfo" + (id != null ? id : ""));
...
function newEvent(id) {
        ...
        if (title != null && title != "" &&
                ...
                firebase.database().ref("events/" + key + "/info").update(info).then(function() {
                        if (id == null) {
                                return firebase.database().ref("events/" + key + "/members").update({
                                        [uid]: 15
                                }).then(function() {
                                        loadEvent(key);
                                });
                        ...
                        firebase.database().ref("events/" + id + "/members/" + uid).once("value", function(me) {
                                try {
                                        if (event.val() == null) {
                                                ...
                                                var contents = [{
                                                        html: "<strong><span style='font-size:3.5vh;color:#2e73f7;'>" + encode(date || "Unknown Date") + "</span></strong>"
                                                }, {
                                                ...
                                                if (event.val().location != null) {
                                                        ...
                                                        body += '</span>';
                                                        contents.push({
                                                                html: body + "<span class='iframe' style='display:none;'><br />" +
                                                ...
                                                contents.push({
                                                        text: event.val().date.duration != null ?
                                                ...
                                                if (member != null) {
                                                        ...
                                                        contents.push({
                                                                html: cb + append + " <span class='okbutton' class='ok" + id + "' onclick='document.querySelectorAll(" + '".okbutton"' +
                                                ...
                                                if (event.val().cancel != null) {
                                                        contents.push({
                                                                html: "<span style='color:red;font-size:3.5vh;'>Cancelled Event</span>"
                                                        });
                                                } else if (event.val().date.time != null) {
                                                        if (new Date(event.val().date.time).getTime() + (event.val().date.duration * 60 * 1000) < new Date().getTime()) {
                                                                contents.push({
                                                                        html: "<span style='color:green;font-size:3.5vh'>Completed Event</span>"
                                                                });
                                                        } else if (new Date(event.val().date.time).getTime() < new Date().getTime()) {
                                                                contents.push({
                                                                        html: "<span style='color:red;font-size:3.5vh;'>Ongoing Event</span>"
                                                                });
                                                        }
                                                }
                                                if (member != null) {
                                                        ...
                                                        link.unshift({
                                                                html: "<span style='font-size:3.5vh;'><a href='#' onclick=" + '"' + href + ';return false;"' + ">Invite</a></span>"
                                                        });
                                                }
                                                if (firstload) {
                                                        loadEventBoard({
                                                ...
                                                if (!firstload) {
                                                        write("Members", [{
                                                                html: "<span class='members'></span>"
                                                        }], links, null, "members" + id, ".members" + id);
                                                } else {
                                                        write("Members", [{
                                                                html: "<span class='members'></span>"
                                                        }], links, null, "members" + id);
                                                }
        ...
        if (document.querySelectorAll(".board" + id).length < 1) {
                write("Event Board", [{
                        html: "<div class='board" + id +
                                ...
                                (member ? ("</div><textarea maxlength='500' placeholder='Type A Message...' " +
                                                ...
                                                if (writes.length == Object.keys(events.val()).length + (invites.val() != null ? Object.keys(invites.val()).length : 0)) {
                                                        ...
                                                        list.forEach(listItem => {
                                                                listItem.forEach(item => {
                                                                        ...
                                                                        var contents = [{
                                                                                html: "<strong><span style='font-size:3.5vh;color:#2e73f7;'>" + encode(item.date.time != Infinity && item.date.time != null ? getFormattedDate(item.date.time) : "Unknown Date") + "</span></strong>"
                                                                        }, {
                                                                                text: address || "Unknown Location"
                                                                        }]; //,{text:duration}];
                                                                        if (item.status.status == 4) {
                                                                                contents.push({
                                                                                        html: "<span style='color:red;font-size:3.5vh;'>Pending Invite</span>"
                                                                                });
                                                                        }
                                                                        if (item.cancel != null) {
                                                                                contents.push({
                                                                        ...
                                                                        if (item.status != null && item.status.info != null) {
                                                                                contents.push({
                                                                                        html: "<span style='color:blue;font-size:3.5vh;'>Updated Info</span>"
                                                                                });
                                                                                border = "orange";
                                                                        }
                                                                        if (item.status != null && item.status.board != null) {
                                                                                contents.push({
                                                                                        html: "<span style='color:blue;font-size:3.5vh;'>" + encode(item.status.board) + " New Message" + (item.status.board > 1 ? "s" : "") + "</span>"
                                                                                });
                                                                                border = "orange";
                                                                        }
...
function leaveEvent(id, cont) {
        changeOns().then(function() {
                firebase.database().ref("users/" + uid + "/events/" + id).update({
                        status: 0
                }).then(function() {
        ...
        if (navigator.onLine) {
                if (!isFacebookApp()) {
						...
						<div class="card" onclick="login('Google')">
							...
							<br /><span style="font-size:2.5vh;">By signing in you agree with the 
							<a href="https://kentonishi.github.io/gatherapp/terms.html">Terms of Service</a> and
							<a href="https://kentonishi.github.io/gatherapp/privacy.html">Privacy Policy</a>.</span><br />
							</img>
						</div>
					`;
                                }
                        ...
                        clear();
                        write("Open App ↗️", [{
                                text: "Tap the menu button, and press " + '"Open With..."' + " to use GatherApp."
                        }]);
                }
        } else {
                clear();
                        ...
                        if (permission === "granted") {
                                navigator.serviceWorker.ready.then(function(reg) {
                                        return reg.pushManager.subscribe({
                                                ...
                                                return firebase.database().ref("users/" + uid + "/subs/").update({
                                                        [key]: subscr
                                                }).then(function() {
                                                        if (id != null) {
...
function iOSSubscribe(e) {
        if (e != null && e != "") {
                firebase.database().ref("users/" + uid + "/subs/").update({
                        [e]: "apns"
                }).then(function() {
                        if (document.querySelectorAll(".enableButton").length > 0) {
                                document.querySelectorAll(".enableButton")[0].innerHTML = "";
...
function write(title, contents, links, href, classlist, overwrite, border) {
        try {
                ...
                if ((title == null && contents != null)) {} else {
